...
// 文章工具函数：从 markdown 源码里提取分类
import { extractCategories } from './posts-utils.js'

// 自动生成侧边栏：扫描 docs/ 目录下的所有 .md 文章（首页 index.md 除外），
// 收集每篇文章 frontmatter 里写的 categories 分类名，侧边栏显示的就是这些分类名。
// 以后新增文章，只要在文章 frontmatter 里写上分类（如 categories: 教程），
// 侧边栏会自动出现这个分类，不用再改这里。
...
function buildSidebar() {
  ...
  // Set 是「不重复的集合」：把站点里出现过的所有分类名收集进来，同一个分类只算一次
  ...
  fs.readdirSync(docsDir)
    ...
    .forEach((file) => {
      ...
      // 没写分类的文章也归入「未分类」，保证它始终有地方展示
      ;(cats.length ? cats : ['未分类']).forEach((cat) => categorySet.add(cat))
    })

  // 侧边栏 = 分类名列表。每个分类一项，点击跳到对应的分类页（/categories/分类名）。
  // 原来的「把所有文章平铺出来」就被这些分类名称替换掉了；
  // 想看某个分类下有哪些文章，进入分类页即可。
  return [...categorySet]
    // 按分类名排序，保证显示顺序稳定（不受文件系统顺序影响）
    .sort((a, b) => a.localeCompare(b))
    .map((category) => ({ text: category, link: '/categories/' + category }))
}
